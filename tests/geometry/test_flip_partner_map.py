...
class TestFlipPartnerMap:
    ...
    def test_square_diagonal_flip_partner(self):
        ...
        assert set(partner) == {1, 2}, f"Flip partner should be vertices 1 and 2, got {partner}"
    ...
    def test_flip_operation(self):
        ...
        assert new_edge == expected_new_edge, f"Expected new edge {expected_new_edge}, got {new_edge}"
        
        # Original edge should no longer be flippable
        assert not flip_map.is_flippable((0, 3)), "Original edge should no longer be flippable"
    ...
    def test_flip_nonflippable_edge_raises_error(self):
        ...
        with pytest.raises(ValueError, match="Edge does not exist in the triangulation"):
            flip_map.flip((0, 1))
    ...
    def test_deep_copy(self):
        ...
        assert copy.compute_triangles() == original.compute_triangles(), "Triangles should be copied"
        ...
        assert original.is_flippable((0, 3)), "Original should still have flippable edge"
    ...
    def test_collinear_points(self):
        ...
        assert len(flip_map.compute_triangles()) == 0, "Collinear points should have no triangles"
        assert len(flip_map.flippable_edges()) == 0, "Should have no flippable edges"
    ...
    def test_multiple_flips(self):
        ...
        if len(initial_flippable) > 0:
            ...
            assert not flip_map.is_flippable(edge1), "Original edge should not be flippable"
            ...
            if len(remaining_flippable) > 0:
                ...
                flip_map.flip(edge2)
                assert not flip_map.is_flippable(edge2), "Second flipped edge should not be flippable"
    ...
    def test_convex_quadrilateral_detection(self):
        ...
        for edge in flippable:
            assert flip_map.is_flippable(edge), f"Edge {edge} should be consistently flippable"
    ...
    def test_flip_updates_triangulation_correctly(self):
        ...
        assert (0, 3) not in flip_map.flip_map, "Original edge should be removed from flip map"
