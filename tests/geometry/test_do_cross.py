...
class TestDoCross:
    ...
    def test_horizontal_vertical_crossing(self):
        ...
        horizontal = Segment(Point(-1, 0), Point(1, 0))  # Horizontal line through origin
        vertical = Segment(Point(0, -1), Point(0, 1))    # Vertical line through origin
        
        assert do_cross(horizontal, vertical), "Horizontal and vertical segments should cross"
    ...
    def test_segments_with_endpoint_on_other_segment_no_crossing(self):
        ...
        assert not do_cross(s1, s2), "Segment with endpoint on other segment should not cross"
    ...
    def test_intersecting_at_endpoint_no_crossing(self):
        ...
        assert not do_cross(base, perpendicular), "T-junction should not be considered crossing"
        
        # L-shape case
        horizontal = Segment(Point(0, 0), Point(1, 0))
        vertical = Segment(Point(1, 0), Point(1, 1))
        
        assert not do_cross(horizontal, vertical), "L-shape should not be considered crossing"
    ...
    def test_segments_that_would_intersect_if_extended(self):
        ...
        assert not do_cross(s1_short, s2_separate), "Segments that don't actually intersect should not cross"
    ...
    def test_crossing_with_rational_coordinates(self):
        ...
        assert do_cross(s1, s2), "Segments with rational coordinates should cross correctly"
    ...
    def test_zero_length_segments(self):
        ...
        assert not do_cross(point_segment, normal_segment), "Zero-length segment should not cross"
    ...
    def test_crossing_with_negative_coordinates(self):
        ...
        assert do_cross(s1, s2), "Segments with negative coordinates should cross correctly"
    ...
    def test_crossing_with_large_coordinates(self):
        ...
        assert do_cross(s1, s2), "Segments with large coordinates should cross correctly"
