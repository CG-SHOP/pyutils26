...
class TestFlippableTriangulation:
    ...
    def test_from_points_edges_valid_triangulation(self):
        ...
        assert triangulation is not None, "Should create FlippableTriangulation successfully"
        ...
        assert len(triangulation._conflicting_edges) == 0, "Should start with no conflicting edges"
    ...
    def test_fork_creates_independent_copy(self):
        ...
        assert fork._flip_map is not original._flip_map, "Fork should have independent flip map"
        
        # Should have same initial state
        assert len(fork._flip_queue) == len(original._flip_queue), "Should have same queue length"
        assert len(fork._conflicting_edges) == len(original._conflicting_edges), "Should have same conflicting edges"
    ...
    def test_add_flip_edge_ordering_normalized(self):
        ...
        assert (3, 0) not in triangulation._flip_queue, "Reversed edge should not be in queue"
    ...
    def test_commit_applies_flips(self):
        ...
        assert triangulation._flip_map.is_flippable((0, 3)), "Edge should be flippable before commit"
        
        # Commit the flip
        triangulation.commit()
        
        # Flip queue should be empty
        assert len(triangulation._flip_queue) == 0, "Flip queue should be empty after commit"
        assert len(triangulation._conflicting_edges) == 0, "Conflicting edges should be cleared"
        
        # Original edge should no longer be flippable
        assert not triangulation._flip_map.is_flippable((0, 3)), "Original edge should not be flippable after commit"
        
        # New edge should be flippable
        assert triangulation._flip_map.is_flippable((1, 2)), "New edge should be flippable after commit"
    ...
    def test_multiple_flips_same_triangulation(self):
        ...
        points = [
            Point(0, 0), Point(2, 0), Point(1, 1), Point(1, -1), Point(0, 2), Point(2, 2)
        ]
        ...
        try:
            ...
            if len(flippable) >= 2:
                ...
                for edge2 in flippable[1:]:
                    if edge2 not in conflicts1 and edge2 not in triangulation._conflicting_edges:
                        triangulation.add_flip(edge2)
                        break
                
                # Should have multiple flips queued
                assert len(triangulation._flip_queue) >= 1, "Should have at least one flip queued"
    ...
    def test_empty_commit_does_nothing(self):
        ...
        assert len(triangulation._conflicting_edges) == 0, "Conflicting edges should remain empty"
        assert triangulation._flip_map.is_flippable((0, 3)), "Original flippability should be unchanged"
    ...
    def test_state_consistency_after_operations(self):
        ...
        assert len(triangulation._conflicting_edges) >= 0, "Should have conflicting edges set"
        
        # Fork and check independence (fork creates a deep copy, so it should be empty initially)
        fork = triangulation.fork()
        assert fork._flip_queue == [], "Fork should start with empty queue (deep copy creates fresh state)"
        assert fork._conflicting_edges == set(), "Fork should start with empty conflicting edges"
        
        # Commit original
        triangulation.commit()
        
        # Original should be empty after commit
        assert len(triangulation._flip_queue) == 0, "Original should be empty after commit"
    ...
    def test_complex_triangulation_operations(self):
        ...
        assert len(triangulation._flip_map.compute_triangles()) == 5, "Should have 5 triangles"
        ...
        if flippable:
            ...
            # Commit
            triangulation.commit()
            assert len(triangulation._flip_queue) == 0, "Queue should be empty after commit"
            ...
            if len(flippable) >= 2:
                ...
                if edge2 and edge2 not in triangulation._conflicting_edges:
                    try:
                        ...
                        assert triangulation._flip_queue[0] == edge1, "First edge should be first in queue"
                        assert triangulation._flip_queue[1] == edge2, "Second edge should be second in queue"
                    except ValueError:
    ...
    def test_error_messages_are_descriptive(self):
        ...
        try:
            ...
            assert "not flippable" in str(e), "Error message should mention flippability"
        ...
        try:
            ...
            assert "triangulation" in str(e), "Error message should mention triangulation"
    ...
    def test_possible_flips_after_adding_flip(self):
        ...
        assert len(possible) == 0, "Should have no possible flips after adding the only flippable edge"
    ...
    def test_possible_flips_excludes_conflicting_edges(self):
        ...
        try:
            ...
            if len(all_flippable) > 0:
                ...
                assert edge_to_flip not in possible, "Added edge should not be in possible flips"
                
                # Any conflicting edges should also not be in possible flips
                conflicts = triangulation._conflicting_edges
                for conflict in conflicts:
                    assert conflict not in possible, f"Conflicting edge {conflict} should not be in possible flips"
    ...
    def test_possible_flips_complex_triangulation(self):
        ...
        assert len(possible) == len(all_flippable), "All flippable edges should be possible initially"
        for edge in all_flippable:
            assert edge in possible, f"Flippable edge {edge} should be in possible flips"
    ...
    def test_possible_flips_return_type_and_format(self):
        ...
        for edge in possible:
            ...
            assert edge[0] < edge[1], "Edge should be normalized (first index < second index)"
    ...
    def test_possible_flips_consistency_with_flip_map(self):
        ...
        assert set(possible) == set(all_flippable), "Possible flips should match all flippable edges initially"
        ...
        if possible:
            ...
            assert edge_to_add not in updated_possible, "Added edge should no longer be possible"
            
            # All remaining possible edges should still be flippable in the map
            for edge in updated_possible:
                assert triangulation._flip_map.is_flippable(edge), f"Edge {edge} should be flippable in map"
    ...
    def test_possible_flips_empty_after_multiple_operations(self):
        ...
        assert len(remaining_possible) == 0, "Should have no possible flips after adding all"
        ...
        for edge in post_commit_possible:
            assert triangulation._flip_map.is_flippable(edge), f"Edge {edge} should be flippable after commit"
